...
from config.config import (GOOGLE_DRIVE_CREDENTIALS, MASTER_SHEET_ID,
...
if sheet_as_list:
    ...
    logger.info(f'----Initial transformation complete: {len(dataframe)} records prepped for PathwaysTransformer.')

    pathways_dataframe = PathwaysTransformer(
        dataframe=dataframe
    ).pathways_transform()
    logger.info(f'----Pathways transformation complete: {len(pathways_dataframe)} records prepped for loading.')
